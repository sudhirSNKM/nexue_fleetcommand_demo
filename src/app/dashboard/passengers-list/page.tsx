...
export default function PassengersListPage() {
  ...
  const handleStatusToggle = (passengerId: string, currentStatus: string) => {
    ...
    toast({
      title: "Access Clearance Updated",
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-[10px] text-white/40 uppercase font-black tracking-[0.4em] mt-1">Global Client Audit & Access Control</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-navy/30 text-white/40 uppercase font-black tracking-widest text-[10px]">
                ...
                <th className="p-4 border-r border-white/5">Comm Links</th>
                <th className="p-4 border-r border-white/5">Rating</th>
                <th className="p-4 border-r border-white/5">Status</th>
                <th className="p-4">Operational Actions</th>
              </tr>
            </thead>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8 ring-1 ring-white/10">
                        ...
                        <AvatarFallback className="bg-navy text-[10px] font-black">{passenger.name[0]}</AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="text-xs font-black">{passenger.name}</p>
                        <p className="text-[8px] text-white/40 font-mono">{passenger.id.substring(0, 12)}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4">
                    <div className="flex flex-col gap-1">
                      <span className="flex items-center gap-1 text-[9px] text-white/60"><Phone className="w-3 h-3" /> {passenger.phone || 'NO LINK'}</span>
                      <span className="flex items-center gap-1 text-[9px] text-white/60"><Mail className="w-3 h-3" /> {passenger.email}</span>
                    </div>
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-1 text-orange">
                      <Star className="w-3 h-3 fill-orange" />
                      <span>{passenger.rating || '5.0'}</span>
                    </div>
                  </td>
                  <td className="p-4">
                    <Badge className={cn(
