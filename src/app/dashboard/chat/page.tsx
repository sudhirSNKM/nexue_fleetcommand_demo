          ...
          <Card className="glass-panel border-none flex-1 flex flex-col overflow-hidden shadow-2xl">
            <CardHeader className="p-3 sm:p-4 bg-navy/20 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xs sm:text-sm font-black uppercase text-white tracking-widest">Command Global</CardTitle>
            ...
            <CardContent className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 scrollbar-hide" ref={scrollRef}>
              {isLoading ? (
                ...
                messages?.map((msg, i) => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn(
                        "max-w-[85%] sm:max-w-[80%] p-3 sm:p-4 rounded-2xl relative shadow-lg",
                        isMe ? "bg-orange text-white rounded-tr-none" : "bg-navy/40 border border-white/5 text-white/90 rounded-tl-none"
                      )}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={cn("text-[7px] sm:text-[8px] font-black uppercase", isMe ? "text-white/60" : "text-orange")}>
                            {msg.senderName} • {msg.senderRole}
                          </span>
                        </div>
                        <p className="text-xs sm:text-sm font-medium leading-relaxed break-words">{msg.text}</p>
                        <span className={cn("text-[6px] sm:text-[7px] font-mono mt-2 block", isMe ? "text-white/40" : "text-white/20")}>
            ...
            <div className="p-3 sm:p-4 bg-navy/20 border-t border-white/5">
              <form onSubmit={handleSendMessage} className="flex gap-2 sm:gap-3">
                <Input 
                  value={newMessage}
                  onChange={(e) => setNewMessage(e.target.value)}
                  placeholder="Transmit tactical intelligence..." 
                  className="bg-navy/40 border-white/10 text-white font-medium h-10 sm:h-12 rounded-xl focus:ring-orange/50 text-xs sm:text-sm"
                />
                <Button type="submit" className="bg-orange hover:bg-orange/90 text-white w-10 h-10 sm:w-12 sm:h-12 rounded-xl p-0 shadow-lg shadow-orange/20 border-none shrink-0">
